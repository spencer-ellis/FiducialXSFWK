...
class InclusiveFiducial( PhysicsModel ):
    ...
    def getYieldScale(self,bin,process):
        ...
        if mergeProductionMode(process) in [ "trueH4e","trueH4mu","trueH2e2mu","trueZ4e","trueZ4mu","trueZ2e2mu"]:
            return 'Sigma_'+mergeProductionMode(process)
        else : return 1
...
class DifferentialFiducial( PhysicsModel ):
    ...
    def getYieldScale(self,bin,process):
        ...
        self.modelBuilder.factory_('expr::%s("@0", r_%s)' % (name, mergeProductionMode(process)))
        if mergeProductionMode(process) in [ "trueH2e2muBin0","trueH4eBin0","trueH4muBin0","trueH2e2muBin1","trueH4eBin1","trueH4muBin1","trueH2e2muBin2","trueH4eBin2","trueH4muBin2","trueH2e2muBin3","trueH4eBin3","trueH4muBin3"]:
            return name
        else :
            return 1
...
class DifferentialFiducialV2( PhysicsModel ):
    ...
    def getYieldScale(self,bin,process):
        ...
        if mergedProcess in Processes: return 'Sigma_'+mergedProcess
        else: return 1
